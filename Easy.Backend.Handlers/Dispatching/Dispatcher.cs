...
namespace Easy.Backend.Handlers.Dispatching
{
    ...
    public class Dispatcher : IDispatcher
    {
        ...
        {
            var handler = SelectHandler(input, context);
            RunHandler(handler, input, context);
        }

        public void DispatchToAllHandlers<TInput, TContext>(TInput input, TContext context)
            where TContext : CancellationTokenExecutionContext
        {
            ...
            var handlers = _serviceLocator.ResolveAll<IHandler<TInput, EmptyObject, TContext>>();
            var tasks = new List<Task>(handlers.Count);

            foreach (var handler in handlers)
            {
                var task = _tasksManager.StartTask(() => RunHandler(handler, input, context), context.Token);
        ...
        private IHandler<TInput, EmptyObject, TContext> SelectHandler<TInput, TContext>(TInput input, TContext context)
            where TContext : IExecutionContext
        {
            ...
            {
                _logger.Error(
                    $"Handler was not found for input '{input}', context '{context}' and resolverHint '{context.ResolverHint}'");
        ...
        private void RunHandler<TInput, TContext>(IHandler<TInput, EmptyObject, TContext> handler, TInput input,
            TContext context)
            where TContext : IExecutionContext
        {
            ...
            catch (Exception e)
            {
                ...
                _logger.Error(
                    $"Error processing input '{input}', context '{context}' and resolverHint '{context.ResolverHint}'", e);
                throw;
            }
